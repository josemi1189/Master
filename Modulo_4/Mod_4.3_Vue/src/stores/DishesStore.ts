...
export const useDishesStore = defineStore(
  ...
  () => {
    ...
    const getIsFavorite = (name: string) =>
      dishes.value.some((dish) => dish.name === name && dish.favorite);
    const getDishById = (id: string) => dishes.value.find((d) => d.id === id);
    const getDishByName = (name: string) =>
      dishes.value.findIndex((d) => d.name === name);
    ...
    const addDish = (
      ...
      if (existingDishIndex === -1) {
        ...
        dishIdToAdd = dishes.value[existingDishIndex]?.id || "";
      }
